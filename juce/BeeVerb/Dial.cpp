...
Dial::TextBox::TextBox()
{
	...
	setColour(juce::Label::outlineWhenEditingColourId, MyColours::red);
}
...
void Dial::setFineInverval(float newFineInterval)
{
...
void Dial::drawDial(juce::Graphics& g)
{
	...
	backgroundArc.addCentredArc(centre.x, centre.y, arcRadius, arcRadius, 0.0f,
		juce::jlimit(startAngle, endAngle, toAngle + space), endAngle, true);
	...
	needle.addRoundedRectangle(centre.x - needleWidth * 0.5f,
		centre.y + needleWidth * 0.5f - needleLen, needleWidth, needleLen, needleWidth * 0.5f);
